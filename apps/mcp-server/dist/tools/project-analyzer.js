...
export class ProjectAnalyzer {
    ...
    async analyzeProject(args) {
        ...
        try {
            ...
            const [structureAnalysis, languageAnalysis, dependencyAnalysis, documentationAnalysis, gitAnalysis,] = await Promise.all([
            ...
            const opportunities = await this.detectDocumentationOpportunities(projectPath, structureAnalysis, languageAnalysis, complexity);
            // Generate recommendations
            const recommendations = this.generateRecommendations(structureAnalysis, languageAnalysis, documentationAnalysis, opportunities);
            const result = {
                project_info: {
                ...
                content: [
                    {
                        ...
                        text: `# Project Analysis Complete\n\n` +
                            ...
                            opportunities.map(opp => `- **${opp.type}** (Priority: ${opp.priority}/10)\n` +
                                `  ${opp.description}\n` +
                                `  Duration: ${opp.estimated_duration}, Value: ${opp.value_score}/10\n`).join('\n') +
                            `\n\n## Key Recommendations:\n\n` +
                            recommendations.slice(0, 5).map(rec => `- **${rec.category}:** ${rec.suggestion} (${rec.impact} impact, ${rec.effort} effort)`).join('\n'),
                    },
                    {
                        type: 'text',
    ...
    async analyzeLanguagesAndFrameworks(projectPath) {
        ...
        try {
            ...
            frameworks = Object.keys(deps).filter(dep => ['react', 'vue', 'angular', 'express', 'fastify', 'next', 'nuxt', 'svelte'].includes(dep));
        }
        catch {
    ...
    async analyzeExistingDocumentation(projectPath) {
    ...
    async calculateComplexity(projectPath, structure) {
    ...
    async detectDocumentationOpportunities(projectPath, structure, language, complexity) {
        const opportunities = [];
    ...
    generateRecommendations(structure, language, documentation, opportunities) {
    ...
    calculateDocumentationCoverage(docFiles) {
        if (docFiles === 0)
            return 'none';
        if (docFiles < 3)
            return 'minimal';
    ...
    async generateInsights(projectPath, depth) {
