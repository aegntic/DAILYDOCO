...
export class PerformanceMonitor extends EventEmitter {
    ...
    async getSystemMetrics(args) {
        ...
        const metrics = await this.collectCurrentMetrics(include_ai_metrics, include_capture_metrics);
            ...
            content: [
                {
                    ...
                    text: `# 🏁 Performance Benchmark Results\n\n` +
                        ...
                        Object.entries(result.comparison.vs_targets).map(([metric, data]) => `- **${metric}:** ${data.achieved.toFixed(2)} / ${data.target.toFixed(2)} (${data.delta > 0 ? '+' : ''}${(data.delta * 100).toFixed(1)}%)\n`).join('') +
    ...
    async collectCurrentMetrics(includeAI, includeCapture) {
    ...
    getHistoricalMetrics(timeRange) {
        ...
        return this.metricsHistory.filter(metric => new Date(metric.timestamp) >= cutoff);
    }
    analyzePerformanceTrends(current, historical) {
        const insights = [];
    ...
    async executeBenchmark(type, duration) {
    ...
    analyzeBenchmarkResults(scores, metrics) {
        const strengths = [];
        const weaknesses = [];
